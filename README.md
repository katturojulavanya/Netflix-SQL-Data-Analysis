...
FROM netflix_titles
WHERE 
type = 'Movie' AND duration = (SELECT MAX(duration) FROM netflix_titles)
```
...
## 9. Count the number of Movies and TV Shows in each genre ##
```
SELECT
type,
listed_in AS 'Genre' ,
COUNT(type) AS 'Total_movies/TVShows'
FROM netflix_titles
GROUP BY
listed_in,
type
...
```
SELECT
COUNT(*) AS salamn_khan_total_movies
FROM netflix_titles
WHERE type = 'Movie'
